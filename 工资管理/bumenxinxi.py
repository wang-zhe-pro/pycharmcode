...
class bumenxinxi:
    def __init__(self, gongzi_window):
        ...
        # 定义下方中心列表区域
        self.columns = ("序号", "部门编号", "部门名称", "部门经理")
        ...
        # 表格的标题
        # 表示列,不显示
        self.tree.column("序号", width=160, anchor='center')
        self.tree.column("部门编号", width=160, anchor='center')
        self.tree.column("部门名称", width=160, anchor='center')
        self.tree.column("部门经理", width=160, anchor='center')
        # 调用方法获取表格内容插入
        ...
        print("test***********************")
        for i in range(min(len(self.id), len(self.num), len(self.name), len(self.manege))):  # 写入数据
            self.tree.insert('', i, values=(
                self.id[i], self.num[i], self.name[i], self.manege[i]))
        ...
        # 定义顶部区域
        # 定义左上方区域
        self.top_title = Label(self.frame_left_top, text="部门信息:", font=('Verdana', 20))
        self.top_title.grid(row=0, column=0, columnspan=2, sticky=NSEW, padx=50, pady=10)

        self.left_top_frame = tk.Frame(self.frame_left_top)
        self.var_id = StringVar()  # 声明序号
        self.var_num = StringVar()  # 声明部门编号
        self.var_name = StringVar()  # 声明部门名称
        self.var_manege = StringVar()  # 声明部门经理
        ...
        # 部门编号
        self.right_top_num_label = Label(self.frame_left_top, text="部门编号：", font=('Verdana', 15))
        ...
        # 部门名称
        self.right_top_name_label = Label(self.frame_left_top, text="部门名称：", font=('Verdana', 15))
        ...
        # 部门经理
        self.right_top_manege_label = Label(self.frame_left_top, text="部门经理：", font=('Verdana', 15))
        self.right_top_manege_entry = Entry(self.frame_left_top, textvariable=self.manege,
                                              font=('Verdana', 15))
        self.right_top_manege_label.grid(row=4, column=0)  # 位置设置
        self.right_top_manege_entry.grid(row=4, column=1)
        ...
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='新建部门信息', width=20, command=self.new_row)
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中部门信息', width=20,
                                            command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='删除选中部门信息', width=20,
                                            command=self.del_row)
        self.right_top_button4 = ttk.Button(self.frame_right_top, text='查询部门信息', width=20,
    ...
    def click(self, event):
        ...
        self.var_manege.set(self.row_info[3])
    ...
    def new_row(self):
        ...
        if self.var_id.get() != '':
            ...
            db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage', autocommit=True)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "select count(1) from department where Did = '%s'" % \
        ...
        if cnt != 0:
            messagebox.showinfo('警告！', '该部门记录已存在！')
        else:
            if self.var_id.get() != '' and self.var_num.get() != '' and self.var_name.get() != '' and self.var_manege.get() != '' :
                # 打开数据库连接
                db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage', autocommit=True)
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO Department(Did, DepNum,DepName, DepManege ) \
				       VALUES ('%s', '%s','%s', '%s','%s' )" % \
                      (self.var_id.get(), self.var_num.get(), self.var_name.get(),
                       self.var_manege.get())  # SQL 插入语句
                try:
                    cursor.execute(sql)  # 执行sql语句
                    db.commit()  # 提交到数据库执行
                except:
                ...
                self.manege.append(self.var_manege.get())

                self.tree.insert('', len(self.id) - 1, values=(
                    self.id[len(self.id) - 1], self.num[len(self.id) - 1],
                    self.name[len(self.id) - 1], self.manege[len(self.id) - 1],))
                self.tree.update()
                messagebox.showinfo('提示！', '插入成功！')
            else:
                messagebox.showinfo('警告！', '请填写部门数据')
    ...
    def updata_row(self):
        ...
        if res == True:
            if self.var_id.get() == self.row_info[0]:  #
                ...
                db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage', autocommit=True)
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "Update Department SET  DepNum = '%s',DepName = '%s', DepManege = '%s'  \
				 WHERE Did = '%s' " %(
                     self.var_num.get(), self.var_name.get(),
                     self.var_manege.get(),self.var_id.get()) # SQL 插入语句
                try:
                    print(sql)
                    cursor.execute(sql)  # 执行sql语句
                    db.commit()  # 提交到数据库执行
                ...
                self.nuum[id_index] = self.var_num.get()
                self.name[id_index] = self.var_name.get()
                self.manege[id_index] = self.var_manege.get()
                ...
                messagebox.showinfo('警告！', '不能修改部门记录！')
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage', autocommit=True)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "DELETE FROM Department WHERE Did = '%s' " % (self.row_info[0])  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
            ...
            del self.num[id_index_index]
            del self.name[id_index_index]
            del self.manege[id_index_index]
    ...
    def select_row(self):
        ...
        db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage', autocommit=True)
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        # sql = "SELECT * FROM drugs"  # SQL 查询语句
        sql = "select * from Department where Did like '%%%%%s%%%%' and DepNum like '%%%%%s%%%%' and DepName like '%%%%%s%%%%'and DepManege like '%%%%%s%%%%' " % \
              (self.var_id.get(), self.var_num.get(), self.var_name.get(),
               self.var_manege.get())  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
