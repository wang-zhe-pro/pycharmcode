...
class jiangchengxize:
    def __init__(self, gongzi_window):
        ...
        # 定义下方中心列表区域
        self.columns = ("序号", "工号", "奖罚方式", "奖罚金额", "奖罚原因")
        ...
        self.tree.column("奖罚方式", width=160, anchor='center')
        self.tree.column("奖罚金额", width=160, anchor='center')
        self.tree.column("奖罚原因", width=160, anchor='center')
        ...
        sql = "SELECT * FROM award"  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
            ...
            for row in results:
                ...
                self.way.append(row[2])
                self.count.append(row[3])
                self.reason.append(row[4])
        except:
        ...
        print("test***********************")
        for i in range(min(len(self.id), len(self.name), len(self.way), len(self.count),
                           len(self.reason))):  # 写入数据
            self.tree.insert('', i, values=(
                self.id[i], self.name[i], self.way[i], self.count[i], self.reason[i]))
        ...
        # 定义顶部区域
        # 定义左上方区域
        self.top_title = Label(self.frame_left_top, text="奖惩细则信息:", font=('Verdana', 20))
        self.top_title.grid(row=0, column=0, columnspan=2, sticky=NSEW, padx=50, pady=10)

        self.left_top_frame = tk.Frame(self.frame_left_top)
        self.var_id = StringVar()  # 声明序号
        self.var_name = StringVar()  # 声明工号
        self.var_way = StringVar()  # 声明奖罚方式
        self.var_count = StringVar()  # 声明奖罚金额
        self.var_reason = StringVar()  # 声明奖罚原因
        ...
        # 工号
        self.right_top_name_label = Label(self.frame_left_top, text="工号：", font=('Verdana', 15))
        ...
        # 奖罚方式
        self.right_top_way_label = Label(self.frame_left_top, text="奖罚方式：", font=('Verdana', 15))
        self.right_top_way_entry = Entry(self.frame_left_top, textvariable=self.var_way,
                                                font=('Verdana', 15))
        self.right_top_way_label.grid(row=3, column=0)  # 位置设置
        self.right_top_way_entry.grid(row=3, column=1)
        # 奖罚金额
        self.right_top_count_label = Label(self.frame_left_top, text="奖罚金额：", font=('Verdana', 15))
        self.right_top_count_entry = Entry(self.frame_left_top, textvariable=self.var_count,
                                                font=('Verdana', 15))
        self.right_top_count_label.grid(row=4, column=0)  # 位置设置
        self.right_top_count_entry.grid(row=4, column=1)

        # 奖罚原因
        self.right_top_reason_label = Label(self.frame_left_top, text="奖罚原因：", font=('Verdana', 15))
        self.right_top_reason_entry = Entry(self.frame_left_top, textvariable=self.var_reason,
                                               font=('Verdana', 15))
        self.right_top_reason_label.grid(row=5, column=0)  # 位置设置
        self.right_top_reason_entry.grid(row=5, column=1)

        # 定义右上方区域
        self.right_top_title = Label(self.frame_right_top, text="操作：", font=('Verdana', 20))

        self.tree.bind('<Button-1>', self.click)  # 左键获取位置
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='新建员工奖惩信息', width=20, command=self.new_row)
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中员工奖惩信息', width=20,
                                            command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='删除选中员工奖惩信息', width=20,
                                            command=self.del_row)
        self.right_top_button4 = ttk.Button(self.frame_right_top, text='清空', width=20,
                                            command=self.clear)
        self.right_top_button5 = ttk.Button(self.frame_right_top, text='查询', width=20,
    ...
    def click(self, event):
        ...
        self.var_way.set(self.row_info[2])
        self.var_count.set(self.row_info[3])
        self.var_reason.set(self.row_info[4])
    ...
    def new_row(self):
        print('123')
        print(self.var_id.get() + self.var_name.get())
        ...
        if self.var_id.get() != '':
            ...
            db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage',
                                 autocommit=True)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "select count(1) from award where Adid = '%s'" % \
        ...
        if cnt != 0:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_way.get() != '' and self.var_count.get() != '' and self.var_reason.get() != '':
                # 打开数据库连接
                db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage', autocommit=True)
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO award(Adid,AwardNum,AwardWay,AwardCount,AwardReason) \
                         VALUES ('%s','%s','%s','%s','%s')" % \
                      (self.var_id.get(), self.var_way.get(), self.var_count.get(),self.var_reason.get(), self.var_name.get() )  # SQL 插入语句
                try:
                    cursor.execute(sql)  # 执行sql语句
                    db.commit()  # 提交到数据库执行
                except:
                ...
                self.way.append(self.var_way.get())
                self.count.append(self.var_count.get())
                self.reason.append(self.var_reason.get())

                self.tree.insert('', len(self.id) - 1, values=(
                    self.id[len(self.id) - 1], self.name[len(self.id) - 1],
                    self.way[len(self.id) - 1], self.count[len(self.id) - 1],
                    self.reason[len(self.id) - 1],))
                self.tree.update()
        ...
        if res == True:
            if self.var_id.get() == self.row[1]:  #
                ...
                db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage',
                                 autocommit=True)
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "Update award SET AwardNum = '%s', AwardWay = '%s',AwardCount = '%s', AwardReason = '%s' \
				 WHERE Aid = '%d' " #% (
                ...
                self.name[id_index] = self.name.get()
                self.way[id_index] = self.var_way.get()
                self.count[id_index] = self.var_count.get()
                self.reason[id_index] = self.var_reason.get()

                self.tree.item(self.tree.selection()[0], values=(
                    self.var_id.get(), self.var_name.get(), self.var_way.get(),
                    self.var_count.get(), self.var_reason.get(),
                    ))  # 修改对于行信息
            else:
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            db = pymysql.connect(host='localhost', user='AloginName', password='root', db='SalaryManage',
                                 autocommit=True)
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "DELETE FROM award WHERE Adid = '%s' " % (self.row_info[0])  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
            ...
            del self.name[_index]
            del self.way[way_index]
            del self.count[count_index]
            del self.reason[reason_index]
